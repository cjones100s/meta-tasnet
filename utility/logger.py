...
class Logger:
    ...
    def log_train_progress(self, epoch: int, losses, num_stages: int, learning_rate: float, progress: int) -> None:
        ...
        guitar = self.multi_stage_to_text([losses[1 + i*4] for i in range(num_stages)])
        ...
        print(f'\repoch {epoch:3d} || train SDR drum {drums} | guitar {guitar} | other {other} | vocal {vocals} | mix {reconstruction:3.1f} | dis {dissimilarity:.3f} | sim {similarity:.3f} | lr {learning_rate:.3e} || {loading_bar} {progress:2d} %', end='', flush=True)
    ...
    def log_train(self, epoch: int, losses, num_stages: int, time_seconds: int, out_file: TextIO) -> None:
        ...
        output = f"epoch {epoch:3d} || train SDR drum {drums} | guitar {guitar} | other {other} | vocal {vocals} | mix {reconstruction:3.1f} | dis {dissimilarity:.3f} | sim {similarity:.3f} || elapsed {time_seconds//60:02d}:{time_seconds%60:02d} || "
    ...
    def log_dev(self, losses, num_stages: int, learning_rate: float, out_file: TextIO) -> None:
        ...
        output = f"dev SDR drum {drums} | guitar {guitar} | other {other} | vocal {vocals} | average {average:4.2f} || lr {learning_rate:.3e}"
